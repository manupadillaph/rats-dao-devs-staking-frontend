...
export function explainErrorTx(errorIn: any): string {
    ...
    if (error === undefined || error === null || error === "" || error == null || error == undefined || error == "null") {
        ...
        if (typeof error === 'string') {
            ...
            if (error.includes("\"GD\"")) { res += sep + "Error getting Datum"; sep = ", "; }
            if (error.includes("\"MF\"") || error == "MF") { res += sep + "Can't find Master information. Maybe the Master didn't fund or already claimed"; sep = ", "; }
            if (error.includes("\"MFGB\"")) { res += sep + "Master already claimed his Funds"; sep = ", "; }
            if (error.includes("\"TOKENS\"")) { res += sep + "Can't create enough Value"; sep = ", "; }
            if (error.includes("\"INT\"")) { res += sep + "Can't find adequate Interest Rate"; sep = ", "; }

            if (error.includes("\"IE\"")) { res += sep + "Can't find Input being validated"; sep = ", "; }
            if (error.includes("\"IE2\"")) { res += sep + "Wrong Input being validated"; sep = ", "; }
            if (error.includes("\"INVOP\"")) { res += sep + "Invalid operation"; sep = ", "; }

            if (error.includes("\"IPD\"")) { res += sep + "Can't find input with PoolDatum"; sep = ", "; }
            if (error.includes("\"INRPD\"")) { res += sep + "Can't find normal or reference input with PoolDatum"; sep = ", "; }
            if (error.includes("\"IRPD\"")) { res += sep + "Can't find reference input with PoolDatum"; sep = ", "; }
            if (error.includes("\"IRPDE\"")) { res += sep + "Can't use reference input with PoolDatum"; sep = ", "; }

            if (error.includes("\"IFDS\"")) { res += sep + "Can't find inputs with FundDatums"; sep = ", "; }
            if (error.includes("\"IFD\"")) { res += sep + "Can't find input with FundDatum"; sep = ", "; }
            if (error.includes("\"ISDS\"")) { res += sep + "Can't find inputs with ScriptDatums"; sep = ", "; }
            if (error.includes("\"IUD\"")) { res += sep + "Can't find input with UserDatum"; sep = ", "; }

            if (error.includes("\"OPD\"")) { res += sep + "Can't find output with PoolDatum"; sep = ", "; }
            ...
            if (error.includes("\"UD\"")) { res += sep + "Wrong UserDatum"; sep = ", "; }

            if (error.includes("\"TPD\"")) { res += sep + "Is PoolDatum"; sep = ", "; }
            if (error.includes("\"TFD\"")) { res += sep + "Is FundDatum"; sep = ", "; }
            if (error.includes("\"TUD\"")) { res += sep + "Is UserDatum"; sep = ", "; }
            if (error.includes("\"TSD\"")) { res += sep + "Is ScriptDatum"; sep = ", "; }
            if (error.includes("\"TOD\"")) { res += sep + "Is OtherDatum"; sep = ", "; }

            if (error.includes("\"PDV\"")) { res += sep + "PoolDatum wrong Value"; sep = ", "; }
            if (error.includes("\"FDV\"")) { res += sep + "FundDatum wrong Value"; sep = ", "; }
            if (error.includes("\"UDV\"")) { res += sep + "UserDatum wrong Value"; sep = ", "; }
            ...
            if (error.includes("\"BTAMT\"")) { res += sep + "Wrong Token Burn amount"; sep = ", "; }
            ...
            if (error.includes("\"MAS\"")) { res += sep + "Missing Minted TxID Master Add Scripts NFT"; sep = ", "; }
            if (error.includes("\"MDS\"")) { res += sep + "Missing Minted TxID Master Delete Scripts NFT"; sep = ", "; }

            if (error.includes("\"UI \"")) { res += sep + "Missing Minted TxID User Deposit NFT"; sep = ", "; }
            if (error.includes("\"UGR\"")) { res += sep + "Missing Minted TxID User Harvest NFT"; sep = ", "; }
            ...
            if (res === "" && error.includes("Error: 400:") && error.includes("maximumExecutionUnits")) {
                res += sep + "Sorry, the transfer uses too many resources!"; sep = ", ";
            }
