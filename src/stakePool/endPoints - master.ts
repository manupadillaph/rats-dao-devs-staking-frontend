...
import {
    ...
    Redeemer_Mint_TxID, ScriptDatum, UserDatum, StakeCredentialPubKeyHash
} from '../types';
import {
    fundID_TN, maxDiffTokensForPoolAndFundDatum, poolDatum_NotTerminated, poolID_TN, scriptID_Master_AddScripts_TN, scriptID_Validator_TN, tokenNameLenght, scriptID_TN, txID_Master_ClosePool_TN, txID_Master_DeleteFund_TN, txID_Master_FundAndMerge_TN, txID_Master_SendBackDeposit_TN, txID_Master_SendBackFund_TN, txID_Master_SplitFund_TN, txID_Master_TerminatePool_TN, userID_TN, poolDatum_NotEmergency, poolDatum_ClaimedFund
...
export async function masterPreparePool(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const isEmergency = poolDatum_NotEmergency
    const minAda = minAda_For_PoolDatum
    //------------------
    const poolDatum_Out = new PoolDatum(masterFunders, fundCount, totalCashedOut, isClosedAt, isTerminated, isEmergency, minAda);
...
export async function masterNewFund(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const poolDatum_Out = mkUpdated_PoolDatum_With_NewFund(poolDatum_In, master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), fundAmount, minAda_For_FundDatum)
    console.log(functionName + " - PoolDatum Out: " + toJson(poolDatum_Out))
    //------------------
    const cashedOut = 0n
    //------------------
    ...
    //------------------
    const redeemer_For_Consuming_Validator_Datum = new Redeemer_Master_Fund(master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), fundAmount, minAda_For_FundDatum)
    //-----------------
...
export async function masterFundAndMerge(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[], assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const poolDatum_Out = mkUpdated_PoolDatum_With_NewFundAmountAndMerging(poolDatum_In, master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), fundAmount, mergingCount)
    console.log(functionName + " - PoolDatum Out: " + toJson(poolDatum_Out))
    //------------------
    ...
    console.log(functionName + " - FundDatum Out: " + toJson(fundDatum_Out))
    //------------------
    const redeemer_For_Consuming_Validator_Datum = new Redeemer_Master_FundAndMerge(master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), fundAmount)
    //-----------------
...
export async function masterSplitFund(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[], assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const poolDatum_In: PoolDatum = eUTxO_With_PoolDatum.datum as PoolDatum
    console.log(functionName + " - PoolDatum In: " + toJson(poolDatum_In))
    const poolDatum_Out = mkUpdated_PoolDatum_With_SplitFundAmount(poolDatum_In, master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), minAda_For_FundDatum_New)
    console.log(functionName + " - PoolDatum Out: " + toJson(poolDatum_Out))
    //------------------
    ...
    //------------------
    const redeemer_For_Consuming_Validator_Datum = new Redeemer_Master_SplitFund(master, new Maybe<StakeCredentialPubKeyHash>(masterStakeCredential), splitFundAmount, minAda_For_FundDatum_New)
    //-----------------
...
export async function masterGetBackFund(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const poolDatum_In: PoolDatum = eUTxO_With_PoolDatum.datum as PoolDatum
    //------------------
    var masterToSendBackStakeCredential
    var masterFunders = poolDatum_In.pdMasterFunders;
    const masterFunder = masterFunders.find(x => x.mfMaster == masterToSendBack);
    if (masterFunder !== undefined) {
        if (masterFunder.mfClaimedFund == poolDatum_ClaimedFund) {
            throw "Master Funder already claimed the funds"; 
        }
        masterToSendBackStakeCredential = masterFunder.mfStakeCredential.val;
    } else {
        throw "Master Funder not found";
    }
    const masterToSendBackAddr = pubKeyHashToAddress(process.env.NEXT_PUBLIC_USE_MAINNET === 'true' ? 1 : 0, masterToSendBack, masterToSendBackStakeCredential);
    console.log(functionName + " - Master To Get Back Addr: " + toJson(masterToSendBackAddr))
    //------------------
...
export async function masterSendBackFund(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets, master_Selected?: Master) : Promise <[string, EUTxO []]> {
    ...
    if (masterToSendBack === undefined) throw "Master to send back is undefined";
    //------------------
    const scriptAddress: Address = poolInfo.scriptAddress
    //------------------
    const uTxOsAtWallet = await lucid!.wallet.getUtxos();
...
export async function masterSendBackDeposit(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const userStakeCredential = userDatum_In.udStakeCredential.val;
    const user_To_SendBackAddr = pubKeyHashToAddress(process.env.NEXT_PUBLIC_USE_MAINNET === 'true' ? 1 : 0, user_To_SendBack, userStakeCredential);
