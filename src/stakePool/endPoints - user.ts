...
import {
    ...
    Redeemer_Burn_TxID, Redeemer_Mint_TxID, Redeemer_User_Deposit, Redeemer_User_Harvest, Redeemer_User_Withdraw, UserDatum, StakeCredentialPubKeyHash
} from '../types';
import {
    ADA_Decimals,
    ADA_UI,
...
export async function userDeposit(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]>  {
    ...
    const userStakeCredential = addrToStakePubKeyHash (userAddr)
    // console.log ("userAddr: " + userAddr)
    // console.log ("PKH: " + user)
    // console.log ("userStakeCredential: " + userStakeCredential)
    // const add2 = pubKeyHashToAddress(process.env.NEXT_PUBLIC_USE_MAINNET === 'true' ? 1 : 0, user, userStakeCredential);
    // console.log ("userAddr2: " + add2)
    //------------------
    ...
    //------------------
    const userDatum_Out = new UserDatum(user, new Maybe<StakeCredentialPubKeyHash>(userStakeCredential), puiInvest, BigInt(createdAt), 0n, 0n, new Maybe<POSIXTime>(), minAda_For_UserDatum_If_StakingIsNotAda);
    console.log(functionName + " - userDatum_Out: " + toJson(userDatum_Out));
    //------------------
    const redeemer_For_Consuming_Validator_Datum = new Redeemer_User_Deposit(user, new Maybe<StakeCredentialPubKeyHash>(userStakeCredential), puiInvest, BigInt(createdAt), minAda_For_UserDatum_If_StakingIsNotAda);
...
export async function userHarvest(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]>{
    ...
    //------------------
    const userDatum_Out: UserDatum = new UserDatum(userDatum_In.udUser, userDatum_In.udStakeCredential, userDatum_In.udInvest, userDatum_In.udCreatedAt, totalRewardsCashedOut, rewardsNotClaimed, new Maybe(BigInt(claimAt)), userDatum_In.udMinAda);
...
export async function userWithdraw(wallet: Wallet, poolInfo: StakingPoolDBInterface, eUTxOs_Selected?: EUTxO[] | undefined, assets?: Assets) : Promise <[string, EUTxO []]> {
    ...
    const user_To_SendBackAddr = pubKeyHashToAddress(process.env.NEXT_PUBLIC_USE_MAINNET === 'true' ? 1 : 0, user, userStakeCredential);
    ...
    if(walletAmount<investAmount_In_UserDatum){
        throw "Not enough UD Tokens in the Wallet to continue with the Withdraw";
    }
    //------------------
    const poolDatum_In: PoolDatum = eUTxO_With_PoolDatum.datum as PoolDatum;
    console.log(functionName + " - PoolDatum In: " + toJson(poolDatum_In));
