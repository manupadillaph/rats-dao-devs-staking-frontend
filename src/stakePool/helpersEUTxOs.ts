...
import { AC, EUTxO, FundDatum, Master_Funder, Maybe, mkFundDatum_FromCbor, mkPoolDatum_FromCbor, mkScriptDatum_FromCbor, mkUserDatum_FromCbor, PoolDatum, POSIXTime, ScriptDatum, StakeCredentialPubKeyHash, UserDatum } from "../types";
...
export function eUTxODBParser(eUTxO: EUTxO | undefined): EUTxO | undefined {
    ...
    if (eUTxO.datum.plutusDataIndex == PoolDatum.plutusDataIndex) {
        ...
        for (var j = 0; j < eUTxO.datum.pdMasterFunders.length; j++) {
            ...
            var mfStakeCredential
            if (eUTxO.datum.pdMasterFunders[j].mfStakeCredential.plutusDataIndex == 0) {
                mfStakeCredential = new Maybe<StakeCredentialPubKeyHash>(eUTxO.datum.pdMasterFunders[j].mfStakeCredential.val)
            } else {
    ...
    if (eUTxO.datum.plutusDataIndex == ScriptDatum.plutusDataIndex) {
        var sdStakeCredential
        if (eUTxO.datum.sdStakeCredential.plutusDataIndex == 0) {
            sdStakeCredential = new Maybe<StakeCredentialPubKeyHash>(eUTxO.datum.sdStakeCredential.val)
        } else {
            sdStakeCredential = new Maybe<StakeCredentialPubKeyHash>()
        }
        eUTxO.datum = new ScriptDatum(eUTxO.datum.sdMaster, sdStakeCredential)
    }
