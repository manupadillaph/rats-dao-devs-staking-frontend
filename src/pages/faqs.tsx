...
const getFaqText = (n : number) => {
	switch(n) {
		...
		case 8: return '<p>Yes! All the validation code/smart contracts are open source and them can be found in the '+process.env.NEXT_PUBLIC_RATS_NAME+' GitHub ('+process.env.NEXT_PUBLIC_RATS_PLUTUS_GITHUB+').</p>\
...
const getFaqTitle = (n : number) => {
	switch(n) {
		case 1: return 'What is '+process.env.NEXT_PUBLIC_CLIENT_PORTAL_NAME+' portal?'
		case 2: return 'How can I use the '+process.env.NEXT_PUBLIC_CLIENT_PORTAL_NAME+' portal?'
		...
		case 9: return 'What are the steps taken to ensure the security of User\'s funds and protect against potential errors or vulnerabilities in the '+process.env.NEXT_PUBLIC_CLIENT_PORTAL_NAME+' portal?'
		default: return 'EXAMPLE Staking portal'
	}
}

const Faqs : NextPage<InferGetServerSidePropsType<typeof getServerSideProps>> = ({} : InferGetServerSidePropsType<typeof getServerSideProps>) =>  {
